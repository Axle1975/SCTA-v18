...
function CreateResources()
    local resourceData = import('worldData.lua')
    local TAutils = import('/lua/TAutils.lua')

    ForkThread(TAutils.WindChangeCall)
    ForkThread(TAutils.TextureChangeCall)

    local terrain = resourceData.GetTerrain()

    local metalAlbedos = terrain.Metal.albedos
    local metalMeshes = terrain.Metal.models
    local metalAmount = terrain.Metal.modelAmount

    local uwMetalAlbedos = terrain.UWMetal.albedos
    local uwMetalMeshes = terrain.UWMetal.models
    local uwMetalAmount = terrain.UWMetal.modelAmount

    local geothermalAlbedos = terrain.Geothermal.albedos
    local geothermalMeshes = terrain.Geothermal.models
    local geothermalAmount = terrain.Geothermal.modelAmount
    ...
    for i, tblData in pairs(markers) do
        if tblData.resource then
            ...
            if tblData.type == "Mass" then
            	CreateDeposit(tblData.type, tblData.position, tblData.size)
                sx = terrain.Metal.Size or 3
                sz = terrain.Metal.Size or 3
                lod = 500
		#Choose a random Metal Patch Graphic
		if tblData.position[2] < GetSurfaceHeight(tblData.position[1],tblData.position[3]) - 5 then
			local metalType = math.random(uwMetalAmount)
			LOG("MetalType: ", metalType)
		        albedo = uwMetalAlbedos[metalType]
	                CreatePropHPR(
			    uwMetalMeshes[metalType],
                	    tblData.position[1], tblData.position[2], tblData.position[3],
	                    0, 0, 0
        	        )
		else
			local metalType = math.random(metalAmount)
			LOG("MetalType: ", metalType)
	        	albedo = metalAlbedos[metalType]
        		CreatePropHPR(
	               	    metalMeshes[metalType],
                	    tblData.position[1], tblData.position[2], tblData.position[3],
        	            0, 0, 0
	               	)
		end
            	#CreateDecal(tblData.position,0,'/env/common/splats/mass_strategic.dds','/env/common/splats/mass_strategic.dds','Albedo',10,10,lod,0,-1,0)
		if not terrain.Metal.Hide then
	            CreateSplat(
                ...
                sz = 4
                lod = 500
		#Choose a random geothermal Graphic
		local geothermalType = math.random(geothermalAmount)
		LOG("GeothermalType: ", geothermalType)
		albedo = geothermalAlbedos[geothermalType]
		CreateResourceDeposit(tblData.type, tblData.position[1],tblData.position[2], tblData.position[3], tblData.size)
	        prop = CreatePropHPR(
		    geothermalMeshes[geothermalType],
                    tblData.position[1], tblData.position[2], tblData.position[3],
                    0, 0, 0
       	        )

		FxSmoke = '/effects/emitters/geothermal_smoke_emit.bp'
		FxSmokeScale = 1
		CreateEmitterAtEntity(prop, -1, FxSmoke):ScaleEmitter(FxSmokeScale)
            CreateSplat(
                tblData.position,           # Position
                0,                          # Heading (rotation)
                albedo,                     # Texture name for albedo
